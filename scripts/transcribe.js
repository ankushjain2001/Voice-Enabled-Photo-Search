...
$('#start-button').click(function () {
    ...
    $('#transcript').val(''); // delete old transcription from search text box
    transcription = ''; // delete old transcription
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            transcript = transcript.replace(/\./g, '').replace(/\,/g, '').toLowerCase(); // remove period and lower case
...
function createPresignedUrl() {
    ...
    return v4.createPresignedURL(
        ...
        crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
            'key': 'AKIAWRTGYOAJYTWCPJPA',
            'secret': '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',
            'sessionToken': '',
            'protocol': 'wss',
            'expires': 15,
            'region': region,
